...
public:
  ...
  void operator=(Type &&) noexcept = default;
};
...
struct QualifiedType {
  ...
  constexpr QualifiedType() noexcept = default;
  constexpr explicit QualifiedType(TypeID typeID, Type::Qualifiers instance_qualifiers = {}) noexcept : derived(typeID.derived), qualifiers(instance_qualifiers), module_id(typeID.module_id), id(typeID.id) {}
  constexpr QualifiedType(TypeID type_id, bool writable) noexcept : QualifiedType(type_id) { qualifiers.writable = writable; }

  edenAlwaysInline [[nodiscard]] constexpr bool isUnqualified() const noexcept { return qualifiers == Type::Qualifiers{}; }
  edenAlwaysInline [[nodiscard]] constexpr TypeID toTypeID() const noexcept { return { .derived = derived, .module_id = module_id, .id = id }; }

  [[nodiscard]] std::string toString() const noexcept { return (qualifiers.writable ? "$ " : ": ") + toTypeID().toString(); }
}; static_assert(sizeof(QualifiedType) == 8);
...
private:
  ...
  static constexpr PrimitiveType primitive_types[num_types] = {
    {I8}, {I16}, {I32}, {I64},
    {U7}, {U15}, {U31}, {U63},
    {U8}, {U16}, {U32}, {U64},
    {F32}, {F64},
...
public:
  constexpr explicit operator QualifiedType() const noexcept {
    QualifiedType t;
    t.derived = PRIMITIVE;
    t.module_id = 0;
...
#define type_singleton(type_name, type_enum) \
  ...
  type_name() noexcept \
  { return primitive_types[std::to_underlying(type_enum)]; }
...
public:
  ...
  edenInlineNodiscardCXPR bool operator==(PointerType other) const noexcept { return eden::are_bitwise_equal_restrict(this, &other); }
};
...
inline constexpr QualifiedType devoid_literal{
  TypeID{
    .derived = Type::DEVOID,
    .module_id = 0,
    .id = 0
...
inline constexpr QualifiedType error_literal{
  TypeID{
    .derived = Type::ERROR,
    .module_id = 0,
    .id = 0
