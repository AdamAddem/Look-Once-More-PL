...
struct SymbolTable {
  struct Function {
    ...
    Function(std::string &&_return_type, std::vector<Types> &&_param_types)
        : return_type(std::move(_return_type)), parameter_types(std::move(_param_types)) {}
  ...
  struct FunctionSignature {
    ...
    void addFunction(std::string && return_type, std::vector<Types> && param_types);

    [[nodiscard]] std::string returnTypeOfCall(const std::vector<Types> &provided_param) const;
  ...
  struct Variable {
    ...
    Variable(Types &&_t, const bool _mut) : type(std::move(_t)), is_mutable(_mut) {}
  ...
  struct TypeDetails {
	...
	struct LocalScope {
		explicit LocalScope(std::string ret_type) : expected_return_type(std::move(ret_type)) {}
  ...
  void            addFunction(const std::string& name, std::string type, std::vector<Types> &&parameter_types);
  bool            containsFunction(const std::string &name) const;
  std::string     returnTypeOfCall(const std::string &name, const std::vector<Types> &provided_params) const;
